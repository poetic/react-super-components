...
class List extends React.Component {
  ...
  sortData(data, sortBy) {
    return _.orderBy(data, sortBy, this.props.sortOrder);
  }
  ...
  checkTypesAndReturnMatchingItemType(index, data) {
    ...
    if (_.isArray(itemTypes)) {
      ...
      if (typesToMatch.includes('default')) {
        const itemType = _.find(itemTypes, { type: 'default' });
  ...
  checkForModificationsAndReturnModifiedData(data) {
  ...
  data: (props, propName, componentName) => {
    ...
    if (_.isArray(props.itemTypes)) {
      data.forEach((dataItem) => {
        if (!_.find(props.itemTypes, { type: 'default' })) {
          if (!_.isObject(dataItem)) {
            throw new Error(
              `Invalid prop \`${propName}\` supplied to \`${componentName}\`. ` +
              `Expected \`${propName}\` to be an array of objects if no default ` +
              'itemType is passed.'
            );
  ...
  itemTypes: (props, propName, componentName) => {
    ...
    if (_.isArray(itemTypes)) {
      itemTypes.forEach((item) => {
        ...
        } else if (item.componentProps && !_.isObject(item.componentProps)) {
          throw new Error(
            `Invalid prop \`${propName}\` supplied to \`${componentName}\`. ` +
            `Expected \`${propName}\` object to have a componentProps property of type Object.`
          );
        }
      });
      ...
      } else if (itemTypes.componentProps && !_.isObject(itemTypes.componentProps)) {
        throw new Error(
          `Invalid prop \`${propName}\` supplied to \`${componentName}\`. ` +
          `Expected \`${propName}\` object to have a componentProps property of type Object.`
        );
